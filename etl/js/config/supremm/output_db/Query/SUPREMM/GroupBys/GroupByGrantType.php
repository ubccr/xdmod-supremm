...
class GroupByGrantType extends \DataWarehouse\Query\SUPREMM\GroupBy
{
   ...
   // special case with two tables that require joins
                                ...
                                $operation, $whereConstraint)
   {
      ...
      $id_field = new \DataWarehouse\Query\Model\TableField($this->granttype_table,  $this->_id_field_name);
      $datatable_account_id_field = new \DataWarehouse\Query\Model\TableField($data_table,'account_id');

      // the where condition that specifies the joins of the tables
      $query->addWhereCondition(new \DataWarehouse\Query\Model\WhereCondition(new \DataWarehouse\Query\Model\TableField($this->account_table,  'id'),
                                                    '=',
                                                    $datatable_account_id_field
                                                    ));
