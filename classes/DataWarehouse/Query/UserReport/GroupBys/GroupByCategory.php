...
class GroupByCategory extends \DataWarehouse\Query\UserReport\GroupBy
{
    public function __construct()
    {
        parent::__construct(
			...
			from modw_supremm.job_category gt
			where 1
			order by gt.id
		"
        );
    ...
    public function pullQueryParameterDescriptions(&$request)
    {
        return parent::pullQueryParameterDescriptions2(
            $request,
            "SELECT description AS field_label FROM modw_supremm.job_category WHERE id IN (_filter_) ORDER BY id"
        );
    }
