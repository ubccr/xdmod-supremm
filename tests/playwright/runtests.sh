...
if [ -n "${log_junit}" ];
then
    PLAYWRIGHT_JUNIT_OUTPUT_NAME=test_results-${log_junit}.xml /xdmod/tests/playwright/node_modules/.bin/playwright  test --reporter=junit tests
else
    /xdmod/tests/playwright/node_modules/.bin/playwright test tests/
fi
